...
# Global pooled HTTP session for Keep-Alive connection reuse
...
def search_products(keyword: str, category: Optional[str] = None, brand: Optional[str] = None, token: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        response = http_session.get(url, headers=_get_headers(token), params=params, timeout=5)
...
def get_product_details(product_id: str, token: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        response = http_session.get(url, headers=_get_headers(token), timeout=5)
        if response.status_code == 200:
            return response.json()
